...
void ReceivedMessage(char* topic, byte* payload, unsigned int length) {
  ...
  for (unsigned int i = 0; i < length; i++) {
    ...
    //command[i] = std::replace( charItem.begin(), charItem.end(), '⸮?%', '');
    //delay(5);
//    String command1 = (String)command;
//    command1 = std::replace( command1.begin(), command1.end(), '⸮', '');
  }
  command[length]='\0';
  Serial.println(command);
  // Handle the message we received
  int result_enroll = strcmp(command,"enroll");
  int result_delete = strcmp(command,"delete");
  ...
  if (result_delete == 0) {
    ...
    empty_database();
    Serial.print("Deleted all finger");
    while (Serial.read() != -1);
  }
}
